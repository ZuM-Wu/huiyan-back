...
(function () {
    ...
    HuiYan.createPage({
        setup() {
            ...
            // 手动清理默认保留 30 天日志，与后端默认值保持一致
            const cleanupDefaultDays = 30;
            const cleanupRetentionDays = ref(cleanupDefaultDays);
            ...
            const openCleanupDialog = () => {
                cleanupRetentionDays.value = cleanupDefaultDays;
                    ...
                    onConfirm: async () => {
                        ...
                        try {
                            ...
                            if (data.deleted_count === 0) {
                                MessagePlugin.warning('没有早于 ' + data.cutoff_time
                                    + ' 的任务日志，保留期内日志未删除');
                            } else {
