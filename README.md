...
慧眼护农是一套**模块化**的智慧农业管理系统，当前版本 `3.4.1`，是**前后端一体的完整项目**：后端基于 FastAPI + 异步 SQLAlchemy + MySQL 8 提供 API 与插件框架，前台覆盖管理端、农户端与站点三端页面；业务能力以可卸载插件（模块）方式组织，并集成 AgentScope AI 对话、Ultralytics 智能识别与系统 MCP。
