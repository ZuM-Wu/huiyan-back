...
async def knowledge_detail(entry_id: int) -> dict:
    ...
    :return: 完整字段字典；条目不存在返回 {"error": ...}
    """
    async with async_session_factory() as db:
        row = await EntryService.get_entry(db, entry_id)
        if not row:
            return {"error": f"知识条目不存在: {entry_id}"}
        names = await CategoryService.name_map(db)
...
async def knowledge_create(
    ...
    async with async_session_factory() as db:
        ...
        if category is None:
            raise ToolError(f"分类不存在: {category_id}")
        entry = KnowledgeEntry(
            title=title.strip(), category_id=category_id, crop=crop or "",
            summary=summary or "", cause=cause or "", solution=solution or "",
            images="", admin_id=0,
        )
        db.add(entry)
        await db.commit()
...
async def knowledge_update(
    ...
    async with async_session_factory() as db:
        ...
        if entry is None:
            raise ToolError(f"知识条目不存在: {entry_id}")
        if category_id:
            category = (await db.execute(
                select(KnowledgeCategory).where(KnowledgeCategory.id == category_id)
            )).scalar_one_or_none()
...
async def knowledge_batch_create(
    ...
    async with async_session_factory() as db:
        ...
        ids = await EntryService.batch_create(
            db, category_id, cleaned, crop or "", 0
        )
        from core.log.active_log import active_log
